...
def deriv_alpha_shift_power_law(t, alpha, delta):
    ...
    :return:
    """
    return (1/(alpha-1) - np.log(t/delta+1)) * normalized_TSPL(t, alpha, delta)
...
def deriv_delta_shift_power_law(t, alpha, delta):
    ...
    :return:
    """
    return (- alpha / (t + delta) + (alpha-1) / delta) * normalized_TSPL(t, alpha, delta)
...
def compute_features_tspl(returns, setting, splitted_parameters):
    ...
    for k, key in enumerate(setting):
        ...
        features[i] = {j: compute_kernel_weighted_sum(x=returns, params=[alphas[k], others[k]],
                                                      func_power_law=normalized_TSPL, transform=power_to(i),
...
def optimal_parameters_from_exponentials_tspl(X, y, p, setting, delta_value, plot=False):
    ...
    if fixed_delta:
        def power_law_with_coef(t, beta, alpha):
            return beta * normalized_TSPL(t, alpha, delta_value)
    else:
        def power_law_with_coef(t, beta, alpha, other):
            return beta * normalized_TSPL(t, alpha, other)
    for i, j0 in setting:
        j = min(j0)
...
def optimal_parameters_GJR_parent(vol, index, optimize_delta=False, delta_value=None,
    ...
    def prediction(parameters, returns, return_features=True,
        ...
        R_1 = compute_kernel_weighted_sum(x=returns, params=parameters[3:],
                                          func_power_law=normalized_TSPL, transform=power_to(1),
    ...
    alpha = opt_params[3]
    delta = opt_params[4]
    weights = normalized_TSPL(np.arange(max_delta) * dt, alpha=alpha, delta=delta)
    ...
    ans = {'opt_params': split_opt_params, 'setting': setting, 'p': p,
           ...
           'train_rmse': root_mean_squared_error(y_true=vol_train, y_pred=vol_pred_train),
           'test_rmse': root_mean_squared_error(y_true=vol_test, y_pred=vol_pred_test),
           ...
           'initial_train_rmse': root_mean_squared_error(y_true=vol_train, y_pred=initial_vol_pred_train),
           'initial_test_rmse': root_mean_squared_error(y_true=vol_test, y_pred=initial_vol_pred_test),
...
def find_optimal_parameters_normalized_tspl(vol, index, p=1, setting=((1, 1), (2, 1/2)), optimize_delta=True, delta_value=None,
                                 train_start_date=train_start_date, test_start_date=test_start_date,
                                 test_end_date=test_end_date, max_delta=1000, fixed_initial=False, use_jacob=True,
...
def fit_arch_model(vol, index, max_delta=200, step_delta=5, train_start_date=train_start_date,
    ...
    test_rmse = root_mean_squared_error(y_true=vol_test, y_pred=test_pred)

    train_pred = model.predict(x)
    train_pred = np.sqrt(np.clip(train_pred, 0, None))

    train_rmse = root_mean_squared_error(y_true=vol_train, y_pred=train_pred)
...
if __name__ == '__main__':
    ...
    sol = find_optimal_parameters_normalized_tspl(vol=vix, index=spx, p=p, setting=setting, train_start_date=train_start,
